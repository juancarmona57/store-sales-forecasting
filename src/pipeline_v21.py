"""V21 Pipeline: Enhanced Multi-Seed Multi-Tweedie Ensemble.

Building on v20 (best Kaggle 0.40305):
1. ALL new P1 features: same-DOW lags, DOW rolling means, trend ratios, promo×DOW
2. Structural zero filtering — removes pre-first-sale noise
3. Improved holiday features: holiday_type ordinal, days_to_christmas, days_to_new_year
4. Same multi-seed multi-Tweedie architecture (5 powers × 3 seeds)
5. Earthquake exclusion + transaction forecasting (proven)
6. Full-data training for final submission (no holdout)
"""
...
# Tweedie variants — v20 showed TW1.0 (31.5%) and TW1.5 (51.3%) dominate
...
def main():
    logger.info("=== V21 PIPELINE: Enhanced Multi-Seed Multi-Tweedie ===")
    logger.info("NEW: same-DOW lags, structural zero filter, trend ratios, holiday features")
    ...
    # Filter structural zeros (remove rows before first sale per store×family)
    train_df, n_removed = filter_structural_zeros(train_df)
    logger.info("Removed %d structural zero rows (pre-first-sale)", n_removed)

    # Build features (now includes same-DOW lags, trend ratios, holiday improvements)
    ...
    logger.info("Building features (v21: +same_dow_lags, +trend_ratios, +holiday_type)...")
    ...
    logger.info("Features: %d train rows, %d test rows, %d feature cols",
                len(train_featured), len(test_featured), len(feature_cols))

    # Log new features specifically
    new_feats = [c for c in feature_cols if any(k in c for k in [
        "dow_3w", "dow_4w", "dow_8w", "dow_52w", "dow_rolling_mean",
        "dow_trend", "dow_yoy", "recent_vs_overall", "promo_x_dow",
        "holiday_type", "days_to_christmas", "days_to_new_year",
    ])]
    logger.info("New P1 features (%d): %s", len(new_feats), new_feats)
    ...
    # Full train (no validation holdout — for final submission)
    ...
    # Memory-efficient: train one at a time, store only predictions
    # =============================================
    logger.info("=== PHASE 1: Training %d models with validation ===",
                len(TWEEDIE_POWERS) * len(SEEDS))

    val_predictions = {}  # key -> numpy array (small)
    individual_scores = {}
    best_iterations = {}
    ...
    # Free individual predictions (keep only variant averages)
    del val_predictions
    gc.collect()
    ...
    logger.info("Equal-weight %d-variant blend: %.6f", n, equal_score)
    ...
    # Memory-efficient: one model at a time
    # =============================================
    ...
    final_pred_opt = sum(best_weights[i] * test_all_preds[i] for i in range(n))
    final_pred_opt = np.clip(final_pred_opt, 0, None)

    final_pred_equal = np.mean(test_all_preds, axis=0)
    final_pred_equal = np.clip(final_pred_equal, 0, None)

    logger.info("Optimized preds: mean=%.1f, median=%.1f, zero_pct=%.1f%%",
                final_pred_opt.mean(), np.median(final_pred_opt),
                (final_pred_opt == 0).mean() * 100)
    logger.info("Equal preds: mean=%.1f, median=%.1f, zero_pct=%.1f%%",
                final_pred_equal.mean(), np.median(final_pred_equal),
                (final_pred_equal == 0).mean() * 100)

    # =============================================
    # SAVE SUBMISSIONS
    # =============================================
    save_opt = SUBMISSIONS_DIR / "submission_v21.csv"
    generate_submission(test_featured, final_pred_opt, save_path=save_opt)
    logger.info("Saved optimized: %s", save_opt)

    save_eq = SUBMISSIONS_DIR / "submission_v21_equal.csv"
    ...
    # =============================================
    logger.info("=" * 60)
    logger.info("V21 RESULTS SUMMARY")
    logger.info("=" * 60)
    ...
    logger.info("")
    logger.info("NEW FEATURES: %d P1 features added", len(new_feats))
    logger.info("STRUCTURAL ZEROS: %d rows removed", n_removed)
    logger.info("=" * 60)
    logger.info("Pipeline v21 complete!")
